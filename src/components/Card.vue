...
const emit = defineEmits(['updateCard', 'deleteCard'])
...
<template>
	...
	<div v-show="isEditing" class="flex flex-col items-center p-4 rounded-lg bg-gray-100 gap-4 dark:bg-gray-800 transition duration cursor-pointer">
		...
		<div class="space-x-2">
			<button @click="cancelEditing" class="px-3 py-1 font-semibold text-sm text-primary-500 border-primary-500 border-2 hover:border-primary-700 hover:text-white hover:bg-primary-700 rounded">
				Cancel
			</button>
			<button @click="saveCardCopy" class="px-3 py-1.5 font-semibold text-sm bg-primary-500 hover:bg-primary-700 rounded text-white">
				Save
			</button>
		</div>
	</div>
